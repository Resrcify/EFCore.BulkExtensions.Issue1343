...
public sealed class PlayerId : ValueObject
{
    ...
    public static Result<PlayerId> Create(string value)
        => Result.Ensure(
            ...
            (v => v.Length <= MaxLength, DomainErrors.PlayerId.ToLong(value, MaxLength)),
            (v => v.Length >= MinLength, DomainErrors.PlayerId.ToShort(value, MinLength)),
            (CanConvertToGuid, DomainErrors.PlayerId.GuidConvertionFailed(value)))
            .Map(e => new PlayerId(e));
